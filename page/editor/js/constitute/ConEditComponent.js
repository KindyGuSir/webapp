...
var IWIN = IWIN || {};
IWIN.EditComponent = {
    type_map : IWIN.Options.comp_map,
    ...
    getMtPsList :function (list,zu){
        ...
        for(var i=0;i<list.length;i++){
            var item = IWIN.util.byId(list[i]);
    ...
    getItemPsStr : function (item){
       ...
       // var strObj = JSON.parse(item.params);
        ...
        strObj.x = IWIN.util.getStyleVal(item,'left');
        strObj.y = IWIN.util.getStyleVal(item,'top');
        strObj.w = IWIN.util.getStyleVal(item,'width');
        strObj.h = IWIN.util.getStyleVal(item,'height');
        strObj.idx = item.style.zIndex;
        strObj.op = item.style.opacity; // 部件的锁定 解锁
        strObj.iH = item.innerHTML;
       // console.log(item.innerHTM+"000000000000000000")
        return strObj;
    },
    parseStrObj :function (o,fn){
        var str = 'prid='+IWIN.Options.cur.program + '&pid='+ IWIN.Options.cur.page;
        var zu = [];
        zu = fn(o,zu);
        // 视频  图片 flash
    ...
    addBlock : function (typeId){
        ...
        el.type_id = typeId;
        el.style.zIndex = IWIN.Level.countZIndex();
    ...
    copyBlock : function (item,id){
        ...
        console.log(el.innerHTML+"---------elelele-----------------------"+item.innerHTML+"---"+id)
