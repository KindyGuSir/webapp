...
var CMDBlockAddAttendance = function (id,x,y){
	id= '29';/*与request.js中data数组中的id的属性值*/
	x=0;
	y=0;
    ...
    var wid=parseInt(params[0])
    var hei=parseInt(params[1])
    IWIN.BlockOpr.setPos(wid/2-300,hei/2-350);
    if(IWIN.BlockOpr.prptBar)IWIN.BlockOpr.prptBar.onBlockDrag(x,y);
    that.elId = item.id;

    IWIN.Request.addBlockReq([item.id]);
};
CMDBlockAddAttendance.prototype = {
    constructor : CMDBlockAddAttendance,
    undo : function (){
