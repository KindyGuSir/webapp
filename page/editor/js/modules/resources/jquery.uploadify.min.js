...
(function(c) {
			...
			this.each(function() {
						...
						if (e[0]) {
							if (e[0] == "*") {
								...
								c("#" + j.queueID).find(".uploadify-queue-item").each(
												function() {
													h++;
													if (e[1] === true) {
	...
	var a = {
		onDialogOpen : function() {
			var d = this.settings;
			this.queueData.errorMsg = "您所上传的文件类型不支持。" /*
														 * "Some files were not
														 * added to the queue:"
														 */;
		...
		onSelect : function(h) {
			...
			for ( var g in this.queueData.files) {
				...
				if (f.uploaded != true && f.name == h.name) {
					...
					if (!e) {
					 ...
					 $.messager.alert('提示',h.name + "文件名称已经存在.", 'info');return ;
				}
			}
			...
			var k = h.name;
			//alert(k + "-----------1");
			k= decodeURIComponent(decodeURI(k));
			//alert(k + "-----------2");
			var obj = k;
			var name=obj.substring(0,obj.lastIndexOf('.'));
			var na=name;
			var md5na =hex_md5(na); // 不带后缀的文件名称
			if (k.length > 25) {
				k = k.substr(0, 25) + "...";
			}
			...
			if (i.itemTemplate == false) {
				i.itemTemplate = '<div id="${fileID}" class="uploadify-queue-item"><span class="fileName">${fileName}(${fileSize})</span><span class="data"></span><div class="cancel"><a id="${fileNa}" style="cursor:pointer" onclick="javascript:$(\'#${instanceID}\').uploadify(\'cancel\', \'${fileID}\',\'1\', \'${filenameall}\')">X</a></div></div>';
			}
		...
		onSelectError : function(d, g, f) {
			...
			if (c.inArray("onSelectError", e.overrideEvents) < 0) {
				...
				case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
					this.queueData.errorMsg += '\nThe file "' + d.name
							+ '" exceeds the size limit (' + e.fileSizeLimit
							+ ").";
					break;
			...
			if (e.checkExisting) {
				c
						.ajax({
							...
							success : function(h) {
								if (h == 1) {
									var g = confirm('A file with the name "'
											+ d.name
