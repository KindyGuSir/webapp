...
IWIN.Level={
    ...
    setAllLevel : function (levelList){
        ...
        {
            el= IWIN.util.byId(levelList[i].id);
    ...
    top:function(itemId){
        ...
        {
            el=IWIN.util.byId(levelList[m].num);
    ...
    moveUp:function(itemId){
        ...
        /*for(var i=levelList.length-1;i>=0;i--){
        	 if( i!=0 && levelList[i].num==itemId ){
        		 ...
        		 console.info(levelList[i].num + " : "+IWIN.util.byId(levelList[i].num).style.zIndex);
        		 console.info(levelList[i-1].num + " : "+IWIN.util.byId(levelList[i-1].num).style.zIndex);
        		 IWIN.util.byId(levelList[i].num).style.zIndex = bz;
        		 IWIN.util.byId(levelList[i-1].num).style.zIndex=az;
        		 console.info(levelList[i].num + " = "+IWIN.util.byId(levelList[i].num).style.zIndex);
        		 console.info(levelList[i-1].num + " = "+IWIN.util.byId(levelList[i-1].num).style.zIndex);
             }
        }*/
        ...
        for(var i=0;i<levelList.length;i++)
        {
            if(levelList[i].num==itemId)
            {
                ...
                if(i!=0)
                {
                    for(var j=0;j<i;j++)
                    {	
                        divCompareOffsetLeft=parseInt((IWIN.util.byId(levelList[j].num).style.left).slice(0,-2));
                        divCompareOffsetTop=parseInt((IWIN.util.byId(levelList[j].num).style.top).slice(0,-2));
                        divCompareOffsetWidth=parseInt(IWIN.util.byId(levelList[j].num).offsetWidth);
                        divCompareOffsetHeight=parseInt(IWIN.util.byId(levelList[j].num).offsetHeight);
        ...
        {
            IWIN.util.byId(levelList[n].num).style.zIndex=levelList[n].zIndex;
        }
    },
        ...
        for(var i=0;i<levelList.length;i++)
        {
            if(levelList[i].num==itemId)
            {
                ...
                if(i!=levelList.length-1)
                {
                    ...
                    {	
                        divCompareOffsetLeft=parseInt(IWIN.util.byId(levelList[j].num).offsetLeft);
                        divCompareOffsetTop=parseInt(IWIN.util.byId(levelList[j].num).offsetTop);
                        divCompareOffsetWidth=parseInt(IWIN.util.byId(levelList[j].num).offsetWidth);
                        divCompareOffsetHeight=parseInt(IWIN.util.byId(levelList[j].num).offsetHeight);
                        if((((divOffsetLeft>divCompareOffsetLeft)&&(divOffsetLeft<(divCompareOffsetLeft+divCompareOffsetWidth)))
                            ||(((divOffsetLeft+divOffsetWidth)>divCompareOffsetLeft)&&(divOffsetLeft<(divCompareOffsetLeft+divCompareOffsetWidth))))
