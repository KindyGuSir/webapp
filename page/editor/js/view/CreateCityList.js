...
(function(win){
	...
	CreateCityList.prototype = {
		...
		getCityList: function(){
			...
			IWIN.util.events(this.arrow, 'click', function(e){
				me.changeUI.call(me);
		...
		addEvent: function(callback){
			...
			IWIN.util.events(this.container, 'click', function(e){
				e = window.event || e;
			...
			// 监听document的click事件
			IWIN.util.events(doc, 'click', function(e){
				callback && callback.call(me);
			});
		},
		pressUpOrDown_handler: function(currkeyCode){  // 方向键（上下键）的处理程序
			...
			for(var i=0,len=total;i<len;i++){
				if(currIndex == i){
					IWIN.util.aClass(items[i],'current');
					continue;
				}
				IWIN.util.dClass(items[i],'current');
			}
		},
		...
		setServerIp: function(serverIp){
			...
			IWIN.BlockOpr.setProperty('serverIp',serverIp);
			IWIN.Controller.execute('CMD_BLOCK_PRPT_CHANGE',IWIN.BlockOpr.getCurObj().id,'serverIp',oldServerIp,serverIp);
		},
		setCity: function(currentCity){
			var oldCity = IWIN.BlockOpr.getProperty('city');
			IWIN.BlockOpr.setProperty('city',currentCity);
			IWIN.Controller.execute('CMD_BLOCK_PRPT_CHANGE',IWIN.BlockOpr.getCurObj().id,'city',oldCity,currentCity);
		},
		...
		destory: function(){
			IWIN.util.delEvent(doc,'click');
			IWIN.util.delEvent(this.container,'click');
			IWIN.util.delEvent(this.arrow,'click');
		}
	}
	
	IWIN.CreateCityList = CreateCityList;
	win.IWIN = IWIN;
})(window);
