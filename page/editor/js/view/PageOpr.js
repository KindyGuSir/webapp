...
IWIN.PageOpr = {
    box:IWIN.Options.el.pageBoxIn,
    pBox : IWIN.Options.el.pageBoxIn.parentNode,
    hintEl : IWIN.Options.el.pgDragHint,
    itemClass:IWIN.Options.eClass.pageItem,
    itemInClass :IWIN.Options.eClass.pgItemIn,
    imgClass : IWIN.Options.eClass.pageImg,
    checkClass : IWIN.Options.eClass.pageCheck,
    util : IWIN.util,
    ...
    creItem : function (obj,idx){
        ...
        foot.className = IWIN.Options.eClass.pageFoot;
        item.appendChild(foot);

        var span = document.createElement('span');
        IWIN.util.setInnerText(span,this.findIdxByItem(item));
        foot.appendChild(span);
        
        var pre = document.createElement('img');
        pre.className = IWIN.Options.eClass.pagePre;
        pre.src='images/hide.png';
        pre.id=obj.pid;
        foot.appendChild(pre);

        var tm = document.createElement('div');
        tm.className = IWIN.Options.eClass.pageTime;
        foot.appendChild(tm);
...
//           IWIN.util.setInnerText(timer,obj.time+'秒');
//           tm.appendChild(timer);
        	
        	var tmEdit = document.createElement('input');
	        tmEdit.className = IWIN.Options.eClass.pageTimeEdit;
	        tmEdit.type="text";
	        ...
	        tmSet.className = IWIN.Options.eClass.pageTimeSet;
    ...
    initPageNum : function (){
        ...
        for(var i=0;i<chs.length;i++){
            ...
            IWIN.util.setInnerText(span,i+1);
        }
    },
    ...
    setCurId : function (id){
        IWIN.Options.cur.page = this.cvt2DataId(id);
        var chs = this.box.children;
        for(var i=0;i<chs.length;i++){
    ...
    renewAllPageIdx : function () {
        ...
        for (var i = 0; i < chs.length; i++) {
            var item = chs[i];
            var foot = item.getElementsByClassName(IWIN.Options.eClass.pageFoot)[0];
    ...
    addPage : function (o,idx){
        ...
        var timeEdit = par.getElementsByClassName(IWIN.Options.eClass.pageTimeEdit);
    ...
    delPage : function (id){
        ...
        delPage.id = IWIN.MakeId.mkPageId();
        IWIN.Options.el.delPgsDiv.appendChild(delPage);
        IWIN.util.moveAllChildren(IWIN.Options.el.mainBoxIn,delPage);
    ...
    mousedwn : function (e){
        ...
        if(IWIN.util.cClass(tar,IWIN.Options.eClass.pageTimeEdit)||IWIN.util.cClass(tar,IWIN.Options.eClass.pageTimeSet)||IWIN.util.cClass(tar,IWIN.Options.eClass.pagePre)){
    ...
    mousemv : function (e){
        ...
        idx=parseInt(document.getElementById("left_page_box").scrollTop/150)+idx
       // console.log(idx + "*******************")
    ...
    focus:function (e){
        ...
        var timeEdit = clickItem.getElementsByClassName(IWIN.Options.eClass.pageTimeEdit)[0];
        IWIN.PageOpr.setTcrId(timeEdit.id);
        
        this.setCurItem(clickItem);
        var newId = clickItem.id.split('_')[1];
        IWIN.Controller.execute('CMD_SELECT_PAGE','',newId);
    },
    blur:function (e){
    	var clickItem = this.findPItem(e);
    	this.setCurItem(IWIN.util.byId(clickItem.id));
    	var that = this;
...
//            else if(edv>86400)edv=86400;
     	...
     	IWIN.PageOpr.setTcrId(null);
        var oldVal = "";
        var newVal = timeEdit.value;
        var item = that.findPItem(clickItem);
        IWIN.Controller.execute('CMD_UPDATE_PAGE_TIME',that.getPageId(item),oldVal,newVal);
    },
    mouseup : function (e){
    	 var tar = e.target || e.srcElement;
    	 // 普通编辑页面 单页预览 读取的是edit目录下的     快速发布页面 单页预览 读取的是publish目录下的
    	 if(IWIN.util.byId('quick').value=='1'){
    		 if(IWIN.util.cClass(tar,IWIN.Options.eClass.pagePre)){
        		 this.setCurItem(IWIN.util.byId("pg_"+tar.id));
             	 IWIN.Controller.execute('CMD_SINGLE_PAGE_PREVIEW_Q',this.getCurItem());
             }
    	 }else{
    		 if(IWIN.util.cClass(tar,IWIN.Options.eClass.pagePre)){
        		 this.setCurItem(IWIN.util.byId("pg_"+tar.id));
             	 IWIN.Controller.execute('CMD_SINGLE_PAGE_PREVIEW',this.getCurItem());
             }
    	 }
        if(!this.dragObj)return;
        ...
        if(endPs.x==this.startPosToBox.x&&endPs.y==this.startPosToBox.y){
            ...
            IWIN.Controller.execute('CMD_SELECT_PAGE',oldId,newId);
        }else{
           //拖放事件
        	...
        	if(IWIN.util.byId('quick').value!='1'){
        		...
        		idx=parseInt(document.getElementById("left_page_box").scrollTop/150)+idx
	            var chs = this.box.children;
	            //console.log(idx + "------chs.length-1=" +(chs.length-1)+ "---------%%%%%%%%%%%%%%%" + endPs.y);
	            if(idx<=chs.length-1){
            	...
            	IWIN.Controller.execute('CMD_MOVE_PAGE',this.cvt2DataId(this.dragItem.id),this.oldIdx,newIdx);
            }
        }
        this.dragItem = null;
